...
classes=sorted(df['weight_class'].dropna().unique())

print('classes',classes)

for wc in classes:
    sub=df[df['weight_class']==wc].reset_index(drop=True)
    if len(sub)<200:
        continue
    print(f'class {wc} size {len(sub)}')
    indices=random.sample(list(sub.index),100)
    test=sub.loc[indices]
    train=sub.drop(indices)
    ...
    train_model(model, train, features, epochs=20, lr=0.001, batch_size=32)
    model.eval()
    with torch.no_grad():
